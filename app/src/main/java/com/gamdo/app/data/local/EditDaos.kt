...
interface CaptureEditStackDao {
    ...
    /**
     * Next free `step_order` for a capture, 1-based.
     *
     * Came from the `Daos.kt` copy of this interface during the main<-p1 merge: both
     * branches declared `CaptureEditStackDao` in the same package, so they were folded
     * into this one. `CaptureRepository.saveEditedCapture` is its caller and is
     * device-verified, so it must not be dropped. Differs from [maxStepOrder] by
     * coalescing the empty case to 0 and returning the *next* slot rather than the last.
     */
    @Query(
        "SELECT COALESCE(MAX(step_order), 0) + 1 FROM capture_edit_stack " +
