...
## 2026-07-28 — 폐쇄 데모 데이터·품질 게이트 조정

- 감도는 공개 서비스가 아닌 폐쇄형 토이프로젝트 데모다. 사용자가 `사진 살리기`를 명시적으로 실행한 입력·마스크·후보 결과·작업 로그는 CAMP-2 GAMDO 작업공간에 수동 정리 전까지 남겨 품질 개선에 사용할 수 있다.
- 입력 즉시 삭제·24시간 자동 삭제·EXIF 제거는 최선 노력의 운영 편의이며, 작업 성공의 선행조건이 아니다.
- InsightFace의 얼굴 수·유사도는 후보 품질을 비교·정렬하는 신호다. 감지기 미검출·임계값 미달만으로 유효한 생성 결과를 숨기거나 로컬 폴백으로 강제하지 않는다. 다만 PNG 디코드, 기대 해상도, 입력 파일 별칭 방지, 아웃페인트의 원본 내부 보존 같은 무결성 검증은 하드 게이트로 유지한다.
- 얼굴·신체를 목표로 바꾸는 별도 기능은 만들지 않는다. 이 제한은 개인정보 보관 정책이 아니라 감도의 제품 범위다.
- 라이브 카메라 프레임 자동 업로드·공개 공유·계정·외부 제3자 API 전송은 추가하지 않는다.

상세 수명·정리 기준은 `감도_데모_데이터_운영계약_2026-07-28.md`를 따른다. 이 절은 이전 기록의 즉시 삭제·엄격한 얼굴 검증 서술보다 우선한다.
