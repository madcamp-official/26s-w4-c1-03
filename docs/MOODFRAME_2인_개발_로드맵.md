...
```text
취향 카드 선택 → 스타일/레퍼런스 선택 → 실시간 시각 오버레이
→ 수동 촬영 → 기본 보정 → 선택형 생성 복구 → 결과 선택·피드백
```
...
| 담당 | 주 책임 | 주요 산출물 |
| --- | --- | --- |
| A - 모바일·카메라 | Kotlin/Compose UI, CameraX, 센서, ML Kit 연동, 오버레이, 수동 촬영, 로컬 보정 | 실제 촬영 가능한 Android 앱 |
...
4. 목표 오버레이에 들어오면 색상 전환으로 상태를 보여주고, 촬영은 사용자가 수동으로 한다.
...
4. **카메라**: 프리뷰, 목표 프레임·실루엣·수평선 오버레이, 표시 토글, 수동 셔터
...
- 하단에는 오버레이 표시 토글과 수동 셔터를 둔다.
- 목표 영역 진입은 오버레이 색 전환으로만 표현한다.
...
| Day 3 | 목표 프레임·실루엣·수평선 오버레이, 표시 토글, 수동 촬영 | 오버레이 좌표 안정화, 이동 평균, 히스테리시스 | 안정적인 시각 오버레이와 수동 촬영 동작 |
...
- Day 3 종료 시점에 오버레이가 불안정하면, 정적 프리셋 프레임으로 다운그레이드하고 가이드 품질을 우선한다.
...
3. 목표 프레임·실루엣·수평선 오버레이를 보고 사용자가 직접 맞춘다.
4. 사용자가 수동 셔터로 촬영한다.
...
- 목표 구도에 맞으면 오버레이 색이 전환되고, 사용자가 수동 촬영한다.
...
- [ ] `StylePreset` JSON 6종과 온보딩 카드 15~20장을 준비한다.
- [ ] CameraX 프리뷰와 사진 저장을 실제 기기에서 확인한다.
- [ ] 얼굴 검출 결과를 카메라 오버레이로 렌더링한다.
- [ ] SQLite 작업 추적 3테이블과 서버 로컬 디스크 저장소를 구성한다.
