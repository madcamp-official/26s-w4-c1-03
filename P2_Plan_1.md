# P2_Plan_1 — 담당 B: 백엔드·CV·생성 (초기 7일 로드맵·역사 기록)

> **⚠️ 범위 대체 고지 (2026-07-28):** 이 문서는 초기 서버·레퍼런스·생성 복구 계획과 구현 이력을 보존한다. 현재 실행 계약은 `remain_plan.md` §0과 `AGENTS.md`를 따른다. 레퍼런스 분석·사진 살리기·생성 복구를 현재 앱 UI에 연결하는 체크박스는 **컷**이며, 서버/GPU 자산은 후속용으로만 보존한다. 현재 P2의 활성 작업은 온디바이스 장면 가이드·성능·로컬 편집 연결 품질이다.
...
- [x] 사진 살리기 모드에서 사용자가 지울 영역을 드래그해 정규화 사각형 마스크를 만든다. <!-- SM-G970N에서 선택 사각형·정규화 masks 전송 확인 -->
...
- [x] 결과 후보를 로컬에 저장하고 사용자가 후보를 선택한 뒤 갤러리에 내보낸다. <!-- SM-G970N에서 AI 결과 2 선택 후 갤러리 저장 확인 -->

#### 카메라 줌 실사용 전환

- [x] CameraX `ZoomState`의 실제 최소·최대 배율을 기준으로 연속 줌을 제공한다.
- [x] 핀치 줌을 프리뷰에 연결하고, 표시·저장 값은 CameraX 실제 `ZoomState`를 기준으로 0.1x 단위 반올림한다. 별도 슬라이더·줌 버튼은 제품 결정에 따라 제공하지 않는다. <!-- 코드·빌드 완료, 직접 두 손가락 입력은 수동 판정 대기 -->
...
- [x] `GET /edit-jobs/{id}`: 읽기 전용 상태 조회. 워커가 `queued → processing → validating → done/fallback`을 갱신하며 폴링은 작업을 종료시키지 않음
- [x] 서버 계약 테스트: queued→processing→done/fallback 전이, 잘못된 jobId 404, 잘못된 요청 422를 `gamdo-server/tests/`에서 검증
- 완료 기준: A의 네트워크 연결 테스트 성공 + 실제 후보 또는 안전한 기본 보정 fallback만 노출되고 고정 이미지는 사용하지 않음
...
- [x] Python 분석 스택 셋업: 배포 범위는 Pillow 기반 메모리 분석으로 고정하고, MediaPipe 보강은 후속 범위로 분리
- [x] 분석 함수 v1: 인물 수, 인물 박스, 얼굴 크기, 인물 점유율, headroom, 수평 추정, 주조색 팔레트, 색온도 추정, 명암 히스토그램
...
- [x] 단위 테스트: 문제 유형별 합성 수치 6케이스 → 기대 진단 <!-- 결과 화면의 사진 살리기 흐름에서 실제 보정과 함께 사용 -->
...
- [x] 휴리스틱 검사: 결과 인물 수 ≠ 기대 인물 수 → 폐기, 극단 색상 변화(히스토그램 거리) → 폐기 <!-- face_count_changed·extreme_color_change 경로와 단위 테스트 완료 -->
- [x] 전 후보 폐기·공급자 미준비 시: `status='fallback'` + failReason 기록 (앱은 기본 보정 유지 — A와 계약된 동작)
- [x] 임베딩은 **메모리에서만 사용 후 폐기** — DB·파일 저장 금지(validation_json에 거리값만) <!-- CAMP-2 buffalo_l CUDA 경로와 fallback을 실제 E2E로 확인 -->
...
- [x] 응답 시간 5초 이내(초과 시 분석 해상도 축소), 타임아웃·비인물 사진(인물 0명) 응답 처리 <!-- 합성 레퍼런스 10장 최대 약 136ms 및 계약 테스트 통과 -->
...
- [x] 생성 안정성 판정: CAMP-2 내부 연속 10건에서 10/10 `done`·`validation=passed` 확인하여 객체 제거 경로 유지
- [x] 판정 결과 기록: `P2_Plan_1.md` 작업 기록의 CAMP-2 부하 검증 항목에 결과·범위를 기록함
...
- [ ] 실제 카드 이미지 15~20장 최종 선정 및 라이선스 확인 — 현재 번들 16개는 출처·허가 증빙이 없어 데모 전용 보류 상태이며, 상세 기록은 `docs/감도_카드_에셋_라이선스_기록.md`에 남김
...
- [x] 데모 환경 네트워크 준비: GAMDO 서비스만 기동하고 CAMP-2 SSH 터널·헬스 체크를 한 번에 수행하는 `gamdo-server/scripts/run_demo.sh` 추가. 핫스팟 조건 자체의 실측 리허설은 발표 장소에서 별도 수행
...
### 2026-07-25 — 사진 살리기 실사용 흐름 완료

- 보정 화면에서 사진 위를 드래그하면 선택 사각형이 표시되고, 선택 영역의 정규화 좌표·면적을 `remove_objects` 마스크로 서버에 전달한다. 기존 고정 20% 탭 영역 방식은 제거했다.
- CAMP-2 SSH 터널을 연결한 `SM-G970N`에서 드래그 마스크 → `방해 요소를 지우는 중…` → 실제 GPU `done` 흐름을 확인했다.
- 결과 2개를 모두 앱 내부 저장소로 다운로드하고 `AI 결과 1/2` 후보로 표시한다. 실기기에서 두 번째 후보를 선택한 뒤 `갤러리에 저장됨`까지 확인했으며, 선택된 GPU 결과가 저장 대상이 되는 것도 확인했다.
...
### 2026-07-25 — 사진 진단 결과 화면 연결

- `ImageMetricsExtractor`를 추가해 원본 Bitmap을 제한된 샘플로 분석하고 밝기·클리핑·라플라시안 분산을 `ProblemDiagnoser` 입력으로 변환한다.
- 결과 화면에 진단 결과를 자연어 칩으로 연결했다. Android JVM 테스트와 Debug APK 빌드는 통과했고, 어두운 테스트 사진을 실기기에 가져와 `조금 어두워요`·`선명도를 확인해보세요` 칩 표시를 화면 캡처로 확인했다.
...
- 전체 해상도 로컬 보정이 끝날 때까지 원본을 숨기던 조건을 `generated ?: edited ?: source`로 변경하고, 조건부 로딩 노드를 명시적 `if`로 분리해 보정 중에도 원본을 표시하고 저장할 수 있게 했다. 최신 APK를 실기기에 재설치해 실제 원본 사진 표시와 로딩 문구 제거를 화면 캡처로 확인했다. 확인용 Debug 로그는 제거했다.

### 2026-07-25 — 개인화 추천 거리 정규화

- `ProfileEngine`의 추천 거리에서 Kelvin 단위의 색온도가 0..1 범위의 다른 차원을 압도하던 문제를 수정했다. 색온도 차이는 2000K 기준으로 정규화하고 나머지 차원은 0..1 범위로 제한한다.
- 구도와 색감이 서로 다른 두 프리셋을 이용한 회귀 테스트를 추가했으며, Android JVM 테스트 36개가 통과했다. `P2ValueDumpTest`의 거리 출력도 정규화 기준으로 갱신했다.

### 2026-07-25 — 진단 칩 실기기 검증

- 기존 번들 사진을 임시로 어둡게 만든 테스트 사진을 Photo Picker로 가져와 결과 화면을 열었다.
- 원본 사진이 정상 표시되고 `조금 어두워요`·`선명도를 확인해보세요` 진단 칩이 실제 화면에 나타나는 것을 확인했다. 테스트용 파일은 기기와 PC에서 삭제했다.

### 2026-07-25 — 카메라 역광 입력 경로 보완

- `ImageConversion.brightnessSample()`을 추가해 Y 평면 한 번 순회로 전체·얼굴·비얼굴 배경 밝기를 계산하고, 회전된 ML Kit 좌표를 분석 프레임에 맞춰 분류한다.
- `CameraScreen`이 가장 큰 얼굴 박스를 샘플러에 전달하도록 연결했다. 기존처럼 전체 밝기만 전달하던 상태를 제거했으며, `FrameFeatureCalculator`의 `backlightFlag`가 실제 입력값을 받을 수 있다.
- `:app:testDebugUnitTest` 및 `:app:assembleDebug` 통과. 역광 장면의 임계값·오탐률은 실제 사진이 필요한 외부 검증 항목으로 후순위 유지한다.

### 2026-07-25 — 잔여 작업 우선순위 재분류

현재 P2 잔여 작업은 다음처럼 분리한다.

- **로컬에서 계속 진행 가능:** 카메라 밝기 샘플러의 회전·영역 경계 회귀 테스트 보강, API 계약·실패 상태 자동 테스트 정리, 정적 분석 설정 검토, 기존 데모 스크립트와 문서의 최신 상태 대조.
- **외부 의존으로 후순위:** 실제 두 손가락 핀치 판정, 오버레이 정렬·색 전환 최종 육안 판정, 실제 인물 5쌍 기반 InsightFace 임계값 캘리브레이션, 큐레이션 사진 품질 판정, 카드 이미지 출처·라이선스 확인, 핫스팟/일반 Wi-Fi 리허설.
...
### 2026-07-25 — 기능명세서 구현 감사 및 P0 결과 흐름 보강

- 전수 대조 결과는 `docs/기능명세서_구현감사_2026-07-25.md`에 모듈 M1~M15별 완료·부분·미구현·확정 결정 대체 항목으로 기록했다. `P1_Plan_1.md`는 변경하지 않았다.
- 이번 반영: 카드 특성 기반 `ProfileEngine` 결과를 `style_profile`·`card_selections`에 저장하고 추천 프리셋을 카메라/결과 기본값에 전달, 결과 화면 6종 프리셋 노출, 1024px 취소 가능한 즉시 프리뷰, 원본↔보정 슬라이더 비교, 저장 시 원본 해상도 렌더링.
- 검증: Android JVM 테스트·Debug APK 빌드, 서버 28개 테스트, SM-G970N 앨범→보정→필터 선택→비교 슬라이더→저장 상태를 통과했다. 신규 온보딩 DB 저장은 기존 실기기 사진을 보존하기 위해 별도 초기화 검증 대기로 유지한다.

### 후속 후보 — 레퍼런스 리믹스(개인 취향 필터)

- 사용자가 원하는 느낌의 사진 1장을 선택하면, 기존 `POST /references/analyze` 결과의 `colorTarget`과 `targetComposition`을 로컬 캐시에 보관하고 **사용자 후보 필터**로 제안한다. 6종 시스템 프리셋을 대체하지 않으며, `내 레퍼런스`는 별도 후보로만 노출한다.
- 촬영 중에는 D2를 지켜 텍스트 지시 없이 기존 시각 오버레이(프레임·실루엣·수평선)에 구도 목표를 반영하고, 촬영 후에는 분석된 색감 파라미터를 로컬 보정에 적용한다. 원본 사진을 복제하거나 고정 결과 이미지로 위장하지 않는다.
- D4·D5를 지켜 별도 계정·서버 저장·API를 만들지 않는다. 업로드는 현행 무저장 분석 요청을 재사용하고, 참조 해시와 분석 결과만 앱 로컬에 저장한다.
- **상태: 후속 아이디어(미구현).** Day 6 이후 신규 기능 금지 원칙에 따라 현재 데모 범위에 추가하지 않으며, 구현 전에는 `ResolvedStyle`/`QuickFilterEditor` 색감 파라미터 변환과 레퍼런스 캐시 만료 정책을 별도로 검증한다.
