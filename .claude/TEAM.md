...
### 9-4. 2026-07-27 · 필터 재작성에서 나온 판정

**"보이지 않는 것"은 "고장난 것"보다 잡기 어렵다.** `QuickFilterEditor`는 예외를 던진 적이 없고, 테스트를 깨뜨린 적이 없고, 기기에서 크래시한 적이 없다. 단지 6개 프리셋 전부가 255단계 중 2단계 미만으로 움직여서 스타일 선택이 항등이었다. 오너가 "있으나 없으나"라고 말하기 전까지 아무 신호도 없었다.

따라서 **지각 가능성은 테스트가 지켜야 하는 속성**이다. `FilterEngineTest`의 하중을 받는 두 테스트는 개별 슬라이더의 정확성이 아니라:

1. 모든 프리셋이 원본과 **눈에 보이게** 다르다 (평균 절대차 > 6 / 255)
2. 모든 프리셋이 **서로** 다르다

나머지 테스트는 이 둘 중 하나가 깨졌을 때 *왜*인지 설명하기 위해 존재한다.

**다른 사람의 수치를 쓸 때는 단위를 바꾸지 마라.** 프리셋 값은 라이트룸 슬라이더 단위(-100~+100, EV)로 저장하고 변환은 엔진 한 곳에서 한다. 정규화해서 저장했으면 출처 글과 한 줄씩 대조할 수 없고, 조용한 리스케일이 같은 출처를 단 다른 룩이 된다. 출처는 `PhotoFilter.credit`에 타입으로 박아 값과 분리될 수 없게 했다.

**단조성은 튜닝이 아니라 구조로 보장하라.** 톤 커브의 각 단계는 항등과 고정 단조 목표 곡선 사이의 볼록 결합이다. 단조 함수의 볼록 결합은 단조이고 합성도 단조이므로, 슬라이더 조합이 어떻든 커브가 접히지 않는다. 첫 구현은 창 함수 `w(y)`로 영역을 마스킹했고 `어두운 영역 +100`에서 0.00→0.320, 0.10→0.315로 **뒤집혔다** — 포스터화이고, 미리보기에서는 원인을 알 수 없다.

**측정 없이 복사할 수 없는 파라미터가 있다.** 톤·색 이동은 사진 사이를 옮겨 다니지만 **노출은 아니다**. 공개된 프리셋의 노출은 그 사진의 노출이다. 상한을 이미지 측정값(p99 휘도의 헤드룸)으로 걸어야 의도는 살고 숫자는 죽는다.

**이미 만들어 둔 것이 배선만 안 돼 있는 경우가 반복된다.** `EditSourceLoader`(inSampleSize 디코드)는 완성돼 있었고 `ResultScreen`은 그 옆에서 `BitmapFactory.decodeFile`로 원본을 통째로 디코드하고 있었다. §5-1 레퍼런스 캐시, §6-2 `ProfileEngine`, §4-2 전후 슬라이더도 같은 상태다. **웨이브를 열기 전에 "이미 있는데 안 불리는 것"부터 확인할 것.**

### 9-5. 2026-07-27 · 수동 보정 패널에서 나온 판정

**엔진이 지원한다는 것과 사용자가 쓸 수 있다는 것은 다르다.** `FilterEngine`은 13개 파라미터를 처리했고 UI는 그중 3개만 노출했다. 나머지 10개는 코드로는 완성돼 있고 화면에서는 존재하지 않았다 — §5-1 레퍼런스 캐시, §6-2 `ProfileEngine`, `EditSourceLoader`와 같은 패턴이다. **"구현했다"를 판정할 때 도달 경로를 함께 본다.**

**죽은 파라미터를 남기지 마라.** `strength`는 기본값 1f로 선언되고 프로덕션에서 한 번도 설정되지 않았다. 테스트만 그것을 썼기 때문에 커버리지는 멀쩡해 보였다. 이제 `Adjustments.strength`로 UI에 나와 있다.

**get/set을 한 곳에 묶어라.** `EditTool`은 라벨·범위·읽기·쓰기·프리셋 기여분을 한 엔트리에 가진다. 대안은 화면에 `when(tool)`이 두 개(읽기용·쓰기용) 생기는 것이고, 그 형태는 **대비를 읽고 채도를 쓰는** 버그를 허용한다 — 각 분기는 따로 보면 옳아서 리뷰로 안 걸린다. 지금은 화면이 `entries` 루프이고 항목별 코드가 0줄이라, 15번째 조정 항목은 enum 한 줄이고 그 외에는 아무 데도 손대지 않는다.

**델타 모델의 대가를 UI가 갚아야 한다.** 사용자 값은 프리셋 위에 더해지는 델타이고 합은 클램프된다. `soft_film`이 밝은 영역을 -100에 고정해 두면 그 방향으로 사용자 슬라이더는 무동작이다. 이건 결함이 아니라 성질이지만, **보이지 않으면 결함과 구분되지 않는다.** 그래서 패널이 `적용 <합계>`를 병기하고, 되찾는 수단(강도)을 같은 스트립 안에 둔다. 테스트 `a_preset_at_its_limit_leaves_no_room_in_that_direction`이 이 성질을 기록으로 고정한다.

**"모든 컨트롤이 픽셀에 닿는다"가 이 패널의 핵심 테스트다.** §9-4의 "모든 프리셋이 눈에 보이게 다르다"와 같은 종류다 — 배선이 빠진 컨트롤은 조용하고, 화면에서는 그저 "이 슬라이더는 효과가 약하네"로 보인다.

### 9-6. 2026-07-27 · 절대값 전환 (오너 지시)

**"필터를 고르면 값들이 필터 값으로 세팅되도록"** — 지시는 한 줄인데 모델이 바뀐다. 델타(프리셋 위에 더함) → 절대값(프리셋이 슬라이더를 세팅함).

이 전환이 같은 날 안에 세 가지를 정리했다.

1. **클램프 사각지대가 소멸했다.** 델타에서는 `soft_film`의 밝은 영역 -100 때문에 그 방향 슬라이더가 죽어 있었다. 어제의 대응은 `적용 <합계>`를 병기해 *설명*하는 것이었는데, 절대값에서는 설명할 것이 없다. **주석으로 완화한 결함은 대개 모델이 틀린 것이다.**
2. **강도가 근거를 잃었다.** 강도를 컷라인에서 되살린 사유가 "클램프된 범위를 되찾는 유일한 수단"이었으므로, 클램프가 없어지면 컨트롤도 없어진다. 하루 안에 컷→복구→재컷이지만 오락가락이 아니다 — **복구 사유가 소멸했기 때문에 재컷했다는 것을 기록으로 남긴다.**
3. **렌더 경로에서 측정 패스가 빠졌다.** 휘도 히스토그램이 필요했던 유일한 이유는 프리셋 노출의 헤드룸 상한이었고, 그건 이제 시딩 시점 1회다. `apply()`는 더 이상 이미지를 재지 않는다.

**절대값에서도 사진 의존 파라미터는 남는다.** 노출은 여전히 사진마다 다르게 시딩된다(`bright_review` 공개 +2.18 EV → 밝은 사진 +26, 어두운 사진 +64 슬라이더 단위). 다만 이제 그 값이 **슬라이더에 보인다** — 렌더는 하나인데 표시가 다른 상태가 없다.

**두 사실에는 두 신호를 쓴다.** 아이콘 색 = 필터가 건드림, 점 = 사용자가 바꿈. 하나로 합치면(예: "0이 아님"만 표시) 필터를 고른 직후 스트립 전체에 표시가 찍혀 정보량이 0이 된다. 테스트 `edited_is_measured_against_the_filter_not_against_zero`가 이걸 고정한다.

**색상 혼합(HSL)만은 필터에 남는다.** 슬라이더가 없는 유일한 항목이라 `apply()`가 필터에서 직접 읽는다. 규칙은 "스칼라는 슬라이더, 색상 혼합은 필터" 한 줄이다.

### 9-7. 2026-07-27 · 디자인 정렬(turn 2)에서 나온 판정

**디자인이 구속 규칙과 충돌하면 오너에게 묻는다.** 2c에는 `AI가 잡은 최적 구도 · 원 포인트`라는 텍스트 안내 카드가 있고, D2는 "텍스트 지시 문구는 만들지 않는다", 규칙 3은 "D2는 오너의 명시적 번복 없이 재론 불가"다. "디자인대로 맞춰"는 D2를 겨냥한 번복이 아니다 — 오너가 그 카드가 D2에 걸린다는 걸 모를 수도 있다. **물어본 결과 D2 유지**였고, 묻지 않았다면 어느 쪽으로 갔든 틀렸을 것이다.

**디자인에 없다고 지우면 안 되는 것이 있다.** 2c에는 가이드 토글도 4:5·1:1 토글도 없다. 지우면 §3-2 체크리스트 항목과 D9의 비율 선택이 **도달 불가**가 된다. 디자인은 화면의 모양을 정하지 목록에서 항목을 빼지 않는다 — 빠진 컨트롤은 디자인 언어로 되살리고, 그 사실을 코드 주석에 남긴다.

**"클린 빌드가 되는가"는 정기적으로 확인해야 한다.** 이번에 `compileDebugKotlin`이 처음으로 실제로 돌자 **HEAD가 컴파일되지 않는다**는 것이 드러났다 — `Daos.kt`↔`ProfileDaos.kt`의 DAO 인터페이스 이중 선언, `GamdoDatabase`의 DAO 접근자 이중 선언, `PresetProfileMapper.kt`↔`ProfileEngine.kt`의 `toPresetProfile` 이중 선언. 셋 다 main←p1 병합의 잔재이고, **Kotlin 증분 컴파일과 KSP 캐시가 계속 이전 결과를 내주고 있었다.** 기기의 APK도 그 캐시 산물이었다. §9-2의 "`📱`는 만료된다"와 같은 종류다 — **초록불이 소스의 상태를 말해 주지 않는 경우가 있다.** 웨이브 종료 시 `clean` 후 1회 빌드를 넣는다.

**이중 선언은 대개 두 개의 서로 다른 결정이다.** `toPresetProfile` 두 벌은 이름만 같았지 정반대였다 — 하나는 `framing`/`candidness`를 프록시로 채우고, 다른 하나는 "방어 가능한 대응물이 없다"며 생략한다. 각자 그 동작을 검증하는 테스트를 달고 있었고, 둘 다 통과했다(둘 다 실제로는 컴파일되지 않았으므로). 중복을 지우는 일은 **어느 결정이 유효한지 고르는 일**이지 정리 작업이 아니다. 여기서는 리드 판정이 문서화된 `PresetProfileMapper.kt`가 정본이고, 옛 하네스 쪽 테스트를 판정에 맞춰 뒤집었다.

**증상을 막는 레이아웃 곡예를 의심하라.** 프리뷰 위아래로 카메라 영상이 새는 문제를 "SurfaceView가 pane 리사이즈를 못 따라온다"로 읽고, pane이 절대 리사이즈되지 않도록 스트립을 띄우고 높이를 예약했다. 두 번 고쳤고 두 번 재발했다. 실제 원인은 **`FILL_CENTER`가 내용을 pane보다 크게 확대하는데 Compose가 interop 뷰의 오버플로를 클립하지 않는 것**이었고, `AndroidView`에 `clipToBounds()` 한 줄로 끝났다. `ImplementationMode.COMPATIBLE`(TextureView)로 바꿔도 아무 변화가 없다는 것이 오진을 갈랐다 — **가설을 반증하는 실험을 먼저 하라.**
